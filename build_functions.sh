...
debootstrap --verbose --no-check-gpg --arch armhf --variant=minbase --foreign ${debian_target_version} ${qemu_mnt_dir} ${debian_mirror_url}
if [ "$?" = "0" ]
then
...
/usr/sbin/chroot ${qemu_mnt_dir} /bin/bash -c "
...
echo '#T0:2345:respawn:/sbin/mingetty ttyS0 115200 vt102' >> /etc/inittab 2>>/deboostrap_stg2_errors.txt	# enable serial console
...
sleep 3
tar_all extract "${output_dir}/tmp/${std_kernel_pkg_name}" "${qemu_mnt_dir}"
sleep 1
sync
chown root:root ${qemu_mnt_dir}/lib/modules/ -R
if [ -e ${qemu_mnt_dir}/lib/modules/gmac_copro_firmware ]
then
	fn_my_echo "Moving gmac-firmware file to the right position ('/lib/firmware')."
	mkdir -p ${qemu_mnt_dir}/lib/firmware/
	mv ${qemu_mnt_dir}/lib/modules/gmac_copro_firmware ${qemu_mnt_dir}/lib/firmware/ 2>>${output_dir}/log.txt
else
	fn_my_echo "Could not find '${qemu_mnt_dir}/lib/modules/gmac_copro_firmware'. So, not moving it."
fi
...
while [ $# -gt 0 ]
do
	echo ${1} >> ${qemu_mnt_dir}/etc/modules 2>>/deboostrap_stg2_errors.txt
	shift
done
fi
...
exit 0" > ${qemu_mnt_dir}/ramzswap_setup.sh
chmod +x ${qemu_mnt_dir}/ramzswap_setup.sh
elif [ "${use_zram}" = "yes" ]
then
echo "#!/bin/sh
...
sed_search_n_replace "mkdir /lib/init/rw/sendsigs.omit.d/" "if [ ! -d  /lib/init/rw/sendsigs.omit.d/ ]; then mkdir /lib/init/rw/sendsigs.omit.d/; fi;" "${qemu_mnt_dir}/etc/init.d/mountkernfs.sh"
...
then
	...
	do
		...
		tar_all extract "${output_dir}/tmp/${extra_files_name}" "${qemu_mnt_dir}"
		if [ "$?" = "0" ]
		then
...
then
	...
	sed -i "s/log_action_msg \"Will now halt\"/\\${led_halt_orange}\n\t&/g" ${qemu_mnt_dir}/etc/init.d/halt  # making the pogoplug's led turn orange on poweroff/shutdown/halt
	sed -i "s/log_action_msg \"Will now restart\"/\\${led_reboot_amber}\n\t&/g" ${qemu_mnt_dir}/etc/init.d/reboot # making the pogoplug's led turn amber on reboot
	
			
	fn_my_echo "Removing the 'qemu-arm-static' binary from the rootfs."
	if [ -e ${qemu_mnt_dir}/usr/bin/qemu-arm-static ]
	then
		rm ${qemu_mnt_dir}/usr/bin/qemu-arm-static
	fi
	
	fn_my_echo "Removing unneccessary file(s) from the rootfs '/tmp' directory."
	rm -rf ${qemu_mnt_dir}/tmp/*
...
if [ -e ${qemu_mnt_dir}/etc/init.d/udev ] && [ -e ${qemu_mnt_dir}/etc/init.d/udev-mtab ]
then
	...
	do
	grep 'tmpfs_size="10M"' "${qemu_mnt_dir}/etc/init.d/${l}"
	if [ "$?" = "0" ]
	then
		sed_search_n_replace "tmpfs_size=\"10M\"" "tmpfs_size=\"${udev_tmpfs_size}\"" "${qemu_mnt_dir}/etc/init.d/${l}" # Change tmpfs size to different value, other than the default 10M. Should be fine for this little system with not much hardware attached and saves RAM!
	else
		fn_my_echo "Not editing '${qemu_mnt_dir}/etc/init.d/${l}'.
...
if [ -e "${qemu_mnt_dir}/etc/init.d/mountoverflowtmp" ]
then
	rm ${qemu_mnt_dir}/etc/init.d/mountoverflowtmp
	if [ "$?" = "0" ]
	then
		fn_my_echo "File '${qemu_mnt_dir}/etc/init.d/mountoverflowtmp' successfully removed."
	else
		fn_my_echo "ERROR while trying to remove file '${qemu_mnt_dir}/etc/init.d/mountoverflowtmp'."
	fi
fi

grep_results=`grep -rn domount ${qemu_mnt_dir}/etc/init.d/ |grep tmpfs`
